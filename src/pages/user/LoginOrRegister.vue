      ...
      >
        ...
        <el-form-item prop="code" class="veri-code">
          <el-input
            ...
            @keyup.enter="toLogin(loginFormRef)"
          ></el-input>
          <img :src="codeUrl" alt="" class="verify-img" @click="getCodeImg" />
        </el-form-item>
        <span class="change-code-img switch-btn" @click="getCodeImg"
          >换一张</span
        >
        <el-button
          type="primary"
      ...
      <el-form :model="register" ref="registerFormRef" :rules="registerRules">
        ...
        <el-form-item prop="veriCode" style="width: 400px" class="email">
          ...
          <el-button
            ...
            >{{ codeBtn.msg }}</el-button
          >
        </el-form-item>
        <el-form-item prop="pwd">
          <el-input
            v-model="register.pwd"
            placeholder="密码（6-16位的数字或字母）"
            type="password"
            show-password
            clearable
            class="input-box"
        ...
        <el-form-item prop="checkPwd">
          <el-input
            ...
            clearable
            class="input-box"
            @keyup.enter="toRegister(registerFormRef)"
          ></el-input>
        </el-form-item>
        <el-button
          type="primary"
      ...
      <el-form :model="register" ref="registerFormRef" :rules="registerRules">
        ...
        <el-form-item prop="checkPwd">
          <el-input
            ...
            clearable
            class="input-box"
            @keyup.enter="forgetPwd(registerFormRef)"
          ></el-input>
        </el-form-item>
        <el-button
          type="primary"
          @click="forgetPwd(registerFormRef)"
          class="confirm-btn"
          >确定</el-button
        >
      </el-form>
...
// 登录  获取图片验证码
function getCodeImg() {
  $API.user.getVerifyCode().then((res) => {
    const blob = res.data;
...
function getEmailCode(type) {
  if (type === 'register') {
    $API.user
      .getRegisterEmailCode({ email: register.email })
      .then(({ data }) => {
        // console.log(data)
        if (data.code === 1000) {
    ...
    $API.user
      .getResetEmailCode({ email: register.email })
      .then(({ data }) => {
        // console.log(data)
        if (data.code === 1000) {
          showMessage('获取成功，请前往邮箱查看。', 'success')
